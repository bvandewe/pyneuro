...
from neuroglia.data.resources.controller import ResourceControllerBase, ReconciliationResult
...
class LabInstanceController(ResourceControllerBase[LabInstanceRequestSpec, LabInstanceRequestStatus]):
        ...
        try:
            ...
            if current_phase == LabInstancePhase.PENDING:
                ...
                return ReconciliationResult.failed(
                    ValueError(f"Unknown phase: {current_phase}"),
    ...
    async def _reconcile_pending_phase(self, resource: LabInstanceRequest) -> ReconciliationResult:
        ...
        if resource.is_scheduled() and not resource.should_start_now():
            ...
            return ReconciliationResult.requeue_after(
                timedelta(minutes=1),
                f"Waiting for scheduled start time: {scheduled_start}"
            )
    ...
    async def _reconcile_provisioning_phase(self, resource: LabInstanceRequest) -> ReconciliationResult:
        ...
        if not container_ready:
            ...
            if datetime.now() - provisioning_start > timedelta(minutes=10):
                ...
                return ReconciliationResult.failed(
                    TimeoutError("Provisioning timeout"),
                    "Container provisioning timed out"
                )
    ...
    async def _reconcile_running_phase(self, resource: LabInstanceRequest) -> ReconciliationResult:
        ...
        if not container_healthy:
            ...
            return ReconciliationResult.failed(
                RuntimeError("Container unhealthy"),
    ...
    async def _transition_to_provisioning(self, resource: LabInstanceRequest) -> None:
        ...
        try:
            ...
            # Start container
            container_info = await self.container_service.create_container(
                template=resource.spec.lab_template,
                resources=allocation,
                environment=resource.spec.environment_variables,
                student_email=resource.spec.student_email
            )
            ...
            condition = LabInstanceCondition(
                ...
                message=f"Allocated resources: {allocation}"
            )
    ...
    async def _transition_to_running(self, resource: LabInstanceRequest) -> None:
        ...
        try:
            ...
            condition = LabInstanceCondition(
                ...
                message=f"Container accessible at: {access_url}"
            )
            resource.status.add_condition(condition)
    ...
    async def _transition_to_completed(self, resource: LabInstanceRequest) -> None:
        ...
        condition = LabInstanceCondition(
            ...
            message="Lab instance completed successfully"
        )
        resource.status.add_condition(condition)
