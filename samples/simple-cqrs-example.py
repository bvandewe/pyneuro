...
from neuroglia.mediation import (
    ...
    SimpleCommandHandler, SimpleQueryHandler,
    add_simple_mediator,
    InMemoryRepository
)
...
class CreateUserHandler(SimpleCommandHandler[OperationResult[UserDto]]):
...
class UpdateUserHandler(SimpleCommandHandler[OperationResult[UserDto]]):
...
class DeactivateUserHandler(SimpleCommandHandler[OperationResult[UserDto]]):
...
class GetUserByIdHandler(SimpleQueryHandler[Optional[UserDto]]):
...
class GetUserByEmailHandler(SimpleQueryHandler[Optional[UserDto]]):
...
class ListActiveUsersHandler(SimpleQueryHandler[list[UserDto]]):
    ...
    async def handle_async(self, query: ListActiveUsersQuery) -> list[UserDto]:
        ...
        return [
            UserDto(user.id, user.name, user.email, user.is_active)
            for user in users
        ]
...
def create_application():
    ...
    # Add simple mediator (no cloud events infrastructure)
