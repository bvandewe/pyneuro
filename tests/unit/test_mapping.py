...
from neuroglia.mapping.mapper import Mapper, MappingConfiguration
from neuroglia.mapping.mapper_profile import MapperProfile
...
class PersonMappingProfile(MapperProfile):
    """Custom mapping profile for person objects"""
    
    def configure(self):
        # Person mapping with custom transformations
        self.create_map(SourcePerson, DestinationPerson) \
            .add_member_mapping(lambda src: src.id, lambda dst: dst.person_id) \
            .add_member_mapping(lambda src: f"{src.first_name} {src.last_name}", lambda dst: dst.full_name) \
            .add_member_mapping(lambda src: src.email, lambda dst: dst.email_address) \
            .add_member_mapping(lambda src: self.calculate_age(src.birth_date), lambda dst: dst.age) \
            .add_member_mapping(lambda src: float(src.salary), lambda dst: dst.annual_salary) \
            .add_member_mapping(lambda src: "Active" if src.is_active else "Inactive", lambda dst: dst.status) \
            .add_member_mapping(lambda src: src.tags, lambda dst: dst.tag_list)
        
        # Address mapping
        self.create_map(SourceAddress, DestinationAddress) \
            .add_member_mapping(lambda src: f"{src.street}, {src.city}", lambda dst: dst.full_address) \
            .add_member_mapping(lambda src: src.postal_code, lambda dst: dst.zip_code) \
            .add_member_mapping(lambda src: src.country, lambda dst: dst.country_name)
        
        # Flattening mapping
        self.create_map(NestedSource, FlattenedDestination) \
            .add_member_mapping(lambda src: src.person.id, lambda dst: dst.person_id) \
            .add_member_mapping(lambda src: f"{src.person.first_name} {src.person.last_name}", lambda dst: dst.person_name) \
            .add_member_mapping(lambda src: src.person.email, lambda dst: dst.person_email) \
            .add_member_mapping(lambda src: src.address.street, lambda dst: dst.address_street) \
            .add_member_mapping(lambda src: src.address.city, lambda dst: dst.address_city) \
...
class TestBasicMapping:
    ...
    def test_mapping_with_computed_properties(self):
        ...
        mapper = Mapper()
        mapping = mapper.create_map(SourcePerson, DestinationPerson)
        mapping.add_member_mapping(
            lambda src: f"{src.first_name} {src.last_name}",
            lambda dst: dst.full_name
        )
        mapping.add_member_mapping(
            lambda src: "Active" if src.is_active else "Inactive",
            lambda dst: dst.status
        )
...
class TestNestedMapping:
    ...
    def test_nested_object_mapping(self):
        ...
        mapper.create_map(SourcePerson, DestinationPerson) \
            .add_member_mapping(lambda src: src.address, lambda dst: dst.home_address)
...
class TestMappingIntegration:
    ...
    def test_complete_mapping_scenario(self):
        ...
        people = [
            SourcePerson(1, "John", "Doe", "john@example.com", date(1990, 1, 1), 
                        Decimal("50000"), True, addresses[0], ["python", "backend"]),
            SourcePerson(2, "Jane", "Smith", "jane@example.com", date(1985, 5, 15), 
                        Decimal("75000"), True, addresses[1], ["frontend", "react"]),
            SourcePerson(3, "Bob", "Johnson", "bob@example.com", date(1992, 8, 20), 
                        Decimal("60000"), False, addresses[2], ["fullstack"])
        ]
