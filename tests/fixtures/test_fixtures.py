...
class CreateTestUserCommandHandler(CommandHandler[CreateTestUserCommand, OperationResult[TestUserDto]]):
    ...
    async def handle_async(self, command: CreateTestUserCommand) -> OperationResult[TestUserDto]:
        try:
            ...
            existing_users = await self.user_repository.find_async(lambda u: u.email == command.email)
            if existing_users:
...
class ListTestUsersQueryHandler(QueryHandler[ListTestUsersQuery, OperationResult[List[TestUserDto]]]):
...
def test_mediator(service_collection, user_repository, user_dto_repository, test_mapper):
    ...
    # Register handlers
    service_collection.add_singleton(CreateTestUserCommandHandler, 
                                   lambda: CreateTestUserCommandHandler(user_repository, test_mapper))
    service_collection.add_singleton(GetTestUserQueryHandler,
                                   lambda: GetTestUserQueryHandler(user_dto_repository))
    service_collection.add_singleton(ListTestUsersQueryHandler,
                                   lambda: ListTestUsersQueryHandler(user_dto_repository))
...
def test_user_dto(test_user_data):
    ...
    return TestUserDto(
        **test_user_data,
        full_name=f"{test_user_data['first_name']} {test_user_data['last_name']}"
    )
